...
- [ETH-USDT-SWAP_1m_live.csv](file://backpack_quant_trading/data/ETH-USDT-SWAP_1m_live.csv)
...
## 更新摘要
**变更内容**   
- 历史数据导入系统重构：移除多交易所CSV处理，简化为Hyperliquid专用数据流
- 新增Hyperliquid专用数据文件支持（ETH_1m_live.csv、ETH-USDT-SWAP_1m_live.csv、ETH_USDC_PERP_1m_live.csv）
- 更新数据导入API路由，优化Hyperliquid数据处理流程
- 增强Hyperliquid WebSocket连接管理和代理支持
...
**更新** 历史数据导入系统已完成重构，移除了多交易所CSV处理功能，现专注于Hyperliquid专用数据流，简化了数据导入流程并提升了系统效率。
...
- 数据层：重构后的Hyperliquid专用数据文件与导入系统
...
HLT["Hyperliquid交易引擎<br/>hyperliquid_trading.py"]
end
subgraph "API 层"
TR["实盘交易API<br/>api/routers/trading.py"]
STR["策略数据API<br/>api/routers/strategy.py"]
end
subgraph "数据层"
CSV1["ETH_1m_live.csv<br/>Hyperliquid专用数据"]
CSV2["ETH-USDT-SWAP_1m_live.csv<br/>Hyperliquid专用数据"]
CSV3["ETH_USDC_PERP_1m_live.csv<br/>Hyperliquid专用数据"]
SYM["symbols_cache.json<br/>交易对缓存"]
end
EC --> DC
EC --> HL
EC --> OS
...
- [ETH_1m_live.csv:1-200](file://backpack_quant_trading/data/ETH_1m_live.csv#L1-L200)
- [ETH-USDT-SWAP_1m_live.csv:1-6](file://backpack_quant_trading/data/ETH-USDT-SWAP_1m_live.csv#L1-L6)
...
- 数据层：重构后的Hyperliquid专用数据文件系统，支持多种数据格式的实时流数据。
...
- WebSocket 支持：增强的WebSocket连接管理，支持代理配置和指数退避重连。
...
**图表来源**
- [hyperliquid_client.py:18-532](file://backpack_quant_trading/core/hyperliquid_client.py#L18-L532)
- [grid_strategy.py:1134-1196](file://backpack_quant_trading/strategy/grid_strategy.py#L1134-L1196)
...
### Hyperliquid专用数据流系统
**更新** 历史数据导入系统已完成重构，移除了多交易所CSV处理，现专注于Hyperliquid专用数据流：

- **数据文件格式**：支持多种Hyperliquid数据格式，包括ETH现货、永续合约等
- **实时数据流**：提供1分钟级别的实时K线数据流
- **数据缓存**：symbols_cache.json提供完整的交易对列表缓存
- **导入优化**：简化了数据导入流程，专注于Hyperliquid数据处理
...
subgraph "支持的数据格式"
G[ETH_1m_live.csv]
H[ETH-USDT-SWAP_1m_live.csv]
I[ETH_USDC_PERP_1m_live.csv]
end
A --> G
A --> H
A --> I
...
**图表来源**
- [strategy.py:180-379](file://backpack_quant_trading/api/routers/strategy.py#L180-L379)
- [ETH_1m_live.csv:1-200](file://backpack_quant_trading/data/ETH_1m_live.csv#L1-L200)
- [ETH-USDT-SWAP_1m_live.csv:1-6](file://backpack_quant_trading/data/ETH-USDT-SWAP_1m_live.csv#L1-L6)
- [ETH_USDC_PERP_1m_live.csv:1-200](file://backpack_quant_trading/data/ETH_USDC_PERP_1m_live.csv#L1-L200)
- [symbols_cache.json:1-543](file://backpack_quant_trading/data/symbols_cache.json#L1-L543)
...
- 数据依赖：重构后的数据层专注于Hyperliquid专用数据处理，简化了多交易所支持。
...
WT --> HL
HLT["HyperliquidTradingEngine"] --> HL
TR["API路由(trading.py)"] --> LT
TR --> WT
STR["策略API(strategy.py)"] --> CSV["Hyperliquid数据文件"]
CSV --> SYM["symbols_cache.json"]
```
...
- **更新** 数据层优化：重构后的Hyperliquid专用数据流减少了不必要的数据转换和存储操作，提升了整体性能。
...
- WebSocket 连接问题
  ...
  - **更新** Hyperliquid WebSocket：支持代理配置，增强连接稳定性。
- 限流与错误码
...
- **更新** 数据导入问题
  - 确认CSV文件路径正确，检查文件编码格式
  - 验证数据格式与列名匹配，确保时间戳格式正确
  - 检查数据库连接和权限配置
...
## 结论
通过 ExchangeClient 抽象与多实现设计，系统实现了对 Backpack、Deepcoin、Hyperliquid、Ostium 的统一接入。结合配置中心、引擎层与 API 层，开发者可在不改动策略层的前提下切换交易所，提升可维护性与扩展性。

**更新** 历史数据导入系统的重构进一步优化了系统架构，移除了多交易所CSV处理的复杂性，专注于Hyperliquid专用数据流，简化了数据处理流程并提升了系统性能。针对各交易所的认证方式、签名规范与链上特性，代码层面提供了完善的实现与健壮的错误处理机制。
...
- 引擎层初始化
  ...
  - **更新** HyperliquidTradingEngine：专门的Hyperliquid交易引擎，支持意图解析和信号自愈
- 日志与状态
  - 通过 API 层查看日志与实例状态，定位问题
- **更新** 数据导入流程
  - 使用策略API导入Hyperliquid专用CSV数据
  - 支持多种数据格式的自动转换和验证
  - 实时数据流支持1分钟级别K线数据
...
### Hyperliquid专用数据文件说明
**更新** 新增的Hyperliquid专用数据文件支持：

- **ETH_1m_live.csv**：标准ETH现货1分钟K线数据
- **ETH-USDT-SWAP_1m_live.csv**：ETH-USDT永续合约1分钟K线数据  
- **ETH_USDC_PERP_1m_live.csv**：ETH-USDC永续合约1分钟K线数据
- **symbols_cache.json**：完整的交易对列表缓存，包含543个交易对

这些文件为Hyperliquid策略回测和实时交易提供高质量的历史数据和实时数据流支持。
