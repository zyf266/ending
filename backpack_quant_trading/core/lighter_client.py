...
class LighterAPIClient:
    ...
    def _is_eth_private_key(self) -> bool:
        ...
        key = self.private_key.lstrip("0x")
        return len(key) == 64  # ETH 私钥
    ...
    async def _get_session(self):
        ...
        # 如果 account_index 是默认小分类值 (<=5)，尝试通过地址自动识别
        if self.account_index <= 5:
    ...
    async def get_price(self, symbol: str, dex: str = "") -> float:
        ...
        try:
            ...
            if inner is not None:
                ...
                if asks:
                    return self._from_price_int(market_index, int(asks[0].price))
                if bids:
                    return self._from_price_int(market_index, int(bids[0].price))
            ...
            logger.warning(f"[Lighter] get_price({symbol}) 无法获取价格, market_index={market_index}, 字段={list(raw.keys())}")
        except Exception as e:
            logger.error(f"[Lighter] get_price({symbol}) 失败: {e}")
        return 0.0
    ...
    async def get_balance(self, dex: str = "") -> float:
        ...
        try:
            ...
            # 正确用法: account(by="index", value="<account_index>")
            account = None
            last_err = None
            ...
            if account is None:
                logger.error(f"[Lighter] account API 调用失败: {last_err}")
                return 0.0
            await api_client.close()
            ...
            logger.error(f"[Lighter] get_balance 异常: {e}")
            return 0.0
    ...
    async def place_order(
        ...
        try:
            ...
            if not reduce_only and leverage > 0:
                try:
                    _, lev_err = await client.update_leverage(
            ...
            if not reduce_only:
                ...
                if current_price <= 0:
                    return {"status": "FAILED", "error": "无法获取 Lighter 实时价格"}
            ...
            if order_type.upper() == "MARKET":
                ...
                # 买入允许最高 cur_price×1.03，卖出允许最低 cur_price×0.97
            ...
            if size_int <= 0:
                return {"status": "FAILED", "error": f"下单数量过小: {coin_size}"}
            ...
            logger.error(f"[Lighter] place_order 异常: {e}")
    ...
    async def place_tpsl_order(
        ...
        **kwargs,
        ...
        try:
            ...
            is_ask = True  # TP/SL 均为平多（SELL）
            size_int  = self._to_size_int(market_index, quantity)
            trig_int  = self._to_price_int(market_index, trigger_price)

            # 唯一客户端订单 ID
            client_order_idx = int(_time.time() * 1000) % (2 ** 31 - 1)

            if is_sl:
                # SL: 触发价=sl_price，限价=sl_price×0.97（确保止损时能成交）
                sl_lim_int = self._to_price_int(market_index, trigger_price * 0.97)
                ...
                # TP: 触发价=tp_price，限价=tp_price×0.99（略低确保成交）
                tp_lim_int = self._to_price_int(market_index, trigger_price * 0.99)
            ...
            logger.info(
                f"[Lighter] {label} 触发单成功: SELL {quantity:.4f} {symbol} "
                f"触发@{trigger_price:.2f}  oid={client_order_idx}"
            )
            return {"status": "FILLED", "orderId": str(client_order_idx)}
        except Exception as e:
            logger.error(f"[Lighter] place_tpsl_order 异常: {e}")
            return {"status": "FAILED", "error": str(e)}
