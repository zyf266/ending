"""
HYPE 2H 做空策略回测 — 对应 Pine Script "自适应做空"（优化版 OR+D + 2H金叉离场）

逻辑：
  空单进场:  (4H MACD死叉[1]  OR  2H收盘下穿日线WMA)  AND  2H MACD[1]偏空(D)
  空单离场:  2H MACD金叉[1]
  止损: +3%  止盈: -10%  锁利: 盈利≥4%时SL锁至+2%  保本: 盈利≥5%时SL至成本

数据源 : Hyperliquid API (HYPE)
进场周期: 2H  |  出场扫描周期: 2H
手续费  : 0.05% 单边（双边0.1%）
"""
...
STOP_LOSS_PCT    = 0.07    # 止损: 入场价 × (1 + 7%)
TAKE_PROFIT_PCT  = 0.03    # 止盈: 入场价 × (1 - 3%)
...
PRICE_FILTER_MAX = 0.0    # 价格下限过滤: 0=不过滤（HYPE价格范围与ETH不同，按需调整）
...
def run_backtest():
    print("=" * 62)
    print(f"  {COIN} 4H进场/1H离场 做空策略回测 (Hyperliquid 数据源)")
    print(f"  进场: 4H  |  出场扫描: 1H  |  方向: SHORT")
    ...
    # ── 计算指标 ──────────────────────────────────────
    df_1h = add_macd(df_1h)            # 1H: MACD（1H金叉离场）
    df_4h = add_macd(df_4h)            # 4H: MACD（进场死叉 + D条件过滤）
    df_1d["wma"] = wma_series(df_1d["c"], WMA_LEN)  # 日线WMA15

    # ── 4H 死叉[1]（进场信号）+ D条件（偏空过滤）────────
    ...
    df_4h["macd_bias"] = df_4h["m_s1"] < df_4h["s_s1"]   # D条件: 4H MACD[1]偏空=True

    # ── 1H MACD 金叉[1]（离场信号）────────────────────
    df_1h["m_s1"] = df_1h["macd"].shift(1)
    df_1h["s_s1"] = df_1h["signal"].shift(1)
    df_1h["m_s2"] = df_1h["macd"].shift(2)
    df_1h["s_s2"] = df_1h["signal"].shift(2)
    ...
    # ── 信号计算 ─────────────────────────────────────
    # ▶ 空单进场：(4H MACD死叉[1]  OR  4H收盘下穿日线WMA)  AND  4H MACD[1]偏空(D)
    df_1h["entry_sig"] = (
    ...
    print(f"\n  进场信号: {df_1h['entry_sig'].sum()}次"
          f"  (dc4h={df_1h['dc4h'].sum()}  cross={df_1h['crossunder_wma'].sum()}  "
          f"macd_bias={df_1h['macd_bias'].sum()}根)")
    print(f"  离场信号: {df_1h['exit_sig'].sum()}次  (1H MACD金叉[1])")
    ...
    # ── 统计汇报 ──────────────────────────────────────
    print(f"\n{'='*62}")
    print(f"📊 回测结果  ·  {COIN} 4H进场/1H离场 做空策略")
    print(f"   期间  : {df_1h['time'].iloc[0].strftime('%Y-%m-%d')}"
    ...
    if not trades:
        ...
        # 导出 CSV
        out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hype_native_short_trades.csv")
