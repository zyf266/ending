      ...
      <el-tab-pane label="交易清单" name="trades">
        <el-card class="chart-card trades-card">
          ...
          <el-table :data="tradesSorted" size="medium" height="600" class="trades-table">
            ...
            <el-table-column prop="position_value" label="仓位价值 USDT" width="140">
              <template #default="{ row }">
                {{ Number(row.position_value).toLocaleString() }}
              </template>
            </el-table-column>
            <el-table-column prop="pnl" label="净损益 USDT" width="130">
              <template #default="{ row }">
            ...
            <el-table-column prop="runup" label="有利波动 USDT" width="130">
              <template #default="{ row }">
            ...
            <el-table-column prop="drawdown" label="不利波动 USDT" width="140">
              <template #default="{ row }">
            ...
            <el-table-column prop="cum_pnl" label="累计P&L USDT" width="150">
              <template #default="{ row }">
          ...
          <div v-if="!trades.length" class="empty">
            暂无回测交易数据，请先调用 /api/strategy/eth-2h/import-csv
          </div>
        </el-card>
      </el-tab-pane>
    </el-tabs>
...
function buildDetailInfoRows() {
  ...
  const basePnls = base.map((t) => Number(t.pnl || 0))
  const basePcts = base.map((t) => Number(t.pnl_pct || 0))
...
/** 根据当前交易列表计算概览指标（与后端口径一致），用于避免缓存导致数据不刷新 */
function computeOverviewFromTrades(tradesArray) {
  ...
  // 按 trade_no 聚合，每笔交易只算一次 pnl，避免进/出两行导致笔数翻倍
...
async function applyDateFilter() {
  ...
  const computed = computeOverviewFromTrades(trades.value)
  if (computed) {
    overview.value = { ...(overview.value || {}), ...computed }
  }
...
async function loadData() {
  ...
  try {
    ...
    const [ovRes, trRes, klRes] = await Promise.all([
      getEthTrendOverview(cacheBust),
      getEthTrendTrades(cacheBust),
      getEthTrendKlines(cacheBust),
    ])
...
function renderEquity() {
  ...
  const initial = 30000000 // 与后端保持一致
  // 使用收益百分比曲线，而不是绝对金额
...
function renderWinLoss() {
  ...
  const losses = base.filter((t) => t.pnl < 0).length
  const flats = base.length - wins - losses
  const total = base.length
