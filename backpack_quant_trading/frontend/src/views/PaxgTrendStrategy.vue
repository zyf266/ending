  ...
  <div class="page strategy-page">
    <div class="title-row">
      <div class="title-main">
        <div class="title-top">
          <h2>沐龙黄金波动率周期捕捉策略 (ML-GVCS)</h2>
          <el-button class="back-link" size="small" link @click="$router.push('/strategies')">返回策略矩阵</el-button>
        </div>
        <p class="sub">
          ML-GVCS 策略专注于黄金（XAU/USD）市场的波动率周期捕捉。其逻辑基于黄金价格波动率的均值回归特性，通过识别波动率从收敛转向扩张的临界点，
          结合宏观趋势过滤，专注在价格回调至关键支撑区域时布局。策略坚持「低位等待、确定性介入」原则，避免追高操作，追求波动率扩张带来的确定性收益。
        </p>
      </div>
      ...
      <el-tab-pane label="交易清单" name="trades">
        <el-card class="chart-card trades-card">
          ...
          <div v-if="!trades.length" class="empty">
            暂无回测交易数据，请先调用 /api/strategy/paxg-2h/import-trades 与 import-klines
          </div>
        </el-card>
      </el-tab-pane>
    </el-tabs>
...
async function applyDateFilter() {
  ...
  // 重新根据当前窗口交易计算概览指标（覆盖后端返回的全区间指标）
  if (trades.value.length) {
    const initial = 2000000
    // 一买一卖算 1 笔：只用出场/止损这一行作为基准
    const exitTrades = trades.value.filter((t) => {
      const tp = String(t.trade_type || '')
      return tp.includes('出') || tp.includes('出场') || tp.includes('止损') || tp.toLowerCase().includes('close')
    })
